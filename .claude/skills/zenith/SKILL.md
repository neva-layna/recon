...
Zenith is initialized in this workspace for Claude Code. If the Zenith MCP server is not visible, restart Claude Code from the workspace root so it reloads `.mcp.json`, then invoke this skill again.

If the workspace has not been initialized, run this from the Zenith checkout:

```bash
uv run zenith init --workspace-dir /path/to/workspace --agent claude
```
...
1. Treat the prompt in `.claude/orchestrator_prompt.md` as the controlling role.
